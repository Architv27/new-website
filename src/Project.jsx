...
import cloud from "../src/assets/pacman.jpg";
import land from "../src/assets/rapidia.jpg";
import rapidia from "../src/assets/solar_system.png";
...
const ProjectsPage = () => {
  ...
  const [isPlaying, setIsPlaying] = useState(true);
      ...
      const slideIntervalTime = 5000; // 5 seconds, or your desired interval

      const manageSlideInterval = () => {
      ...
      }, [slides.length, dialogOpen, isPlaying]);
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timeoutId);
      ...
      const goToNextSlide = () => {
        ...
        manageSlideInterval(); // Reset interval
      };
  ...
  return (
    <div className={`slideshow-container ${dialogOpen ? 'blur-background' : ''}`}>
      ...
      <IconButton className="arrow right-arrow" style={{width:"40px", height:"40px", zIndex:150, color:"whitesmoke"}} onClick={goToNextSlide}>
        <span>&gt;</span>
      </IconButton>
      </div> 
      {slides.map((slide, index) => (
          <animated.div key={index} className={`slide ${index === currentSlide ? 'active' : 'inactive'}`}
            ...
            <animated.div {...bind()} className="slide-title" onClick={() => {setCurrentSlide(index);handleSlideClick(currentSlide)}} style={{ x: dragProps.x, transform:"translate(-50%,-50%)", fontFamily:"sans-serif" }}>
              {slideTitles[index].split(" ").map((word, wordIndex) => (
                <span key={wordIndex} className={wordIndex === 1 ? 'highlight' : ''}>{word} </span>
              ))}
            </animated.div>
          </animated.div>
        ))}
      ...
      />
      <IconButton onClick={togglePlayPause} style={{ position: 'absolute', bottom: '10px', right: '5px', zIndex: 150, color:"#ff1818"}}>
        {isPlaying ? <PauseIcon style={{ width:40, height:40 }} /> : <PlayArrowIcon style={{ width:40, height:40 }} />}
      </IconButton>
